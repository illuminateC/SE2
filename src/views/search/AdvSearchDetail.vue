<template>
  <!-- 搜索详情页的总体页面 -->
  <div class="adv-search-detail-container clearfix">
    <!-- 上部分，高级检索的卡片 -->
    ...
    <div class="result-area" v-show="!advanceCardExpanded">
...
const advanceSecondLevelFilter = reactive({
  "works": [
    ...
    {
      label: "论文标题",
      value: "display_name.search",
    },
    {
      label: "论文摘要",
      value: "abstract.search",
    },
    {
      label: "论文内容",
  ...
  "authors": [
    {
      label: "机构",
      value: "last_known_institution.display_name",
    },
    {
      label: "机构所属国家",
...
const advanceFilterResult = reactive({
  ...
  "authors": [
    // 默认含有 机构 和 领域 两个项目
    {
      category: "last_known_institution.display_name",
      content: "",
      logicalType: AND_LABLE_VALUE,
    },
    {
      category: "x_concepts.display_name",
      content: "",
      logicalType: AND_LABLE_VALUE,
    },
  ],
  ...
  "concepts": [
    ...
    {
      category: "ancestors.display_name",
      content: "",
      logicalType: AND_LABLE_VALUE,
    },
    {
 ...
 * (对象类型)以便于发送给后端
 * DEBUG 2022.12.12
 * 这里高级检索需要把高级检索部分的筛选字段也合并到这里，因此这里 将用户已经勾选的条目构建出 filter 对象
 * 拼接在 高级检索中的筛选字段后
 * @return 一个对象
 */
...
/**
 * DEBUG 2022.12.12 高级检索这里触发的函数不是 handleFinalSearch 而是 handleAdvanceSearch()
 * 切换搜索实体类型时
